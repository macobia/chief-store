...
const OrderSummary = () => {
	...
	// 		const { link } = res.data;
	// 		if (link) {
	// 			window.location.href = link; // Redirect user to Flutterwave payment link
	// 		} else {
    ...
    const handlePayment = async () => {
        try {
          ...
          const {  tx_ref, totalAmount, redirect_url, logo, flutterwavePublicKey } = res.data;
    
          // 👇 You can open payment directly OR redirect to payment link
