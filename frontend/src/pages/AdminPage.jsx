...
import { BarChart, PlusCircleIcon, ShoppingBasket, Bell } from "lucide-react";
    ...
    <div className='min-h-screen relative overflow-hidden'>
      ...
      <div className="mx-30 my-4  ">
        {/* <button onClick={() => setShowNotifications(!showNotifications)} className="relative"> */}
         <button onClick={toggleNotifications} className="relative cursor-pointer">
      ...
      <tbody>
        {orders.map((order) => (
          <tr key={order._id} className="border-b border-gray-700">
             ...
             <td className="p-2 text-white">{order.flutterwaveSessionId}</td>
              ...
              {order.orderStatus === 'pending' ? (
                <>
                  ...
                  <button
                    ...
                    onClick={() => updateOrderStatus(order._id, 'decline')}
                  >
                    Decline
                  </button>
                </>
              ) : (
                <span className={`px-2 py-1 rounded ${order.orderStatus=== 'success' ? 'bg-green-500' : 'bg-red-500'}`}>
                  {order.orderStatus}
                </span>
              )}
            </td>
        ...
        <div className='flex justify-center mb-8'>
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center px-4 py-2 mx-2 rounded-md transition-colors duration-200 cursor-pointer ${activeTab === tab.id
